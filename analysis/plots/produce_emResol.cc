...
////////////////////////////////////////////////////////////////////
void produce_emResol(string BeamType, string rotation, string interaction, string Option = "") {
  ...
  EMResolution* resolution = nullptr;
  ...
  if(BeamType == "em")
  {
    ...
    if(Option.find("m4") != string::npos) {
      centerTower = "M4-T2";
      printf("[produce_emResol] Using M4-T2 as center tower.\n");
    } else {
      printf("[produce_emResol] Using M5-T2 as center tower (default).\n");
    }
    
    resolution = new EMResolution(inputPath, centerTower, BeamType.c_str());
  } 
  else if(BeamType == "pi")     {
    TString centerTower = "M5-T2";
    resolution = new EMResolution(inputPath, centerTower, BeamType.c_str());
  ...
  else {throw std::invalid_argument("[produce_emResol] BeamType = " + BeamType + " is not recognized");}
  ...
  if(Option.find("nnr") != string::npos)
  {
    ...
    printf("[produce_emResol] NoNoise option is enabled. Fitting without noise term up to 110 GeV.\n");
  }
