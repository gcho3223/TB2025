...
// Function to visualize module configuration based on module_position.txt
void printModuleConfiguration(const std::vector<std::pair<int, int>>& modulesToProcess, const std::string& moduleCenter, const std::string& size)
{
    std::cout << "Module layout (# = selected, . = not selected):" << std::endl;
    std::cout << "#upstream view" << std::endl;
    
    // Create a set for faster lookup
    std::set<std::string> selectedModules;
    for(const auto& moduleInfo : modulesToProcess)
    {
        std::string moduleStr = "M" + std::to_string(moduleInfo.first) + "T" + std::to_string(moduleInfo.second);
        selectedModules.insert(moduleStr);
    }
    
    // Define the 6x6 grid layout based on module_position.txt
    ...
    for(int row = 0; row < 6; row++)
    {
        ...
        {
            std::string module = layout[row][col];
            if(selectedModules.find(module) != selectedModules.end()) {std::cout << " # ";}
            else {std::cout << " . ";}
        }
        std::cout << std::endl;
    }
    
    //std::cout << "Selected modules:" << std::endl;
    //for(const auto& moduleInfo : modulesToProcess) {std::cout << "  M" << moduleInfo.first << "T" << moduleInfo.second << std::endl;}
    //std::cout << std::endl;
}

float DRCorrection(float Edep_S, float Edep_C)
{
  float chi = 0.291;
  float drc = (Edep_S - chi*Edep_C) / (1-chi);
  return drc;
}
