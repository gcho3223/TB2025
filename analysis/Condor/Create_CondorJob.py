import os
import sys

print("Usage: python Create_CondorJob.py particle:[calib|em|pi|kp] program:[ats|dwc|evt] rotation:[on|off] interaction target:[on|off]")
if len(sys.argv) < 5:
    print("Usage: python Create_CondorJob.py particle:[calib|em|pi|kp] program:[ats|dwc|evt] rotation:[on|off] interaction target:[on|off]")
    sys.exit(1)

particle = sys.argv[1]
program = sys.argv[2]
rotation_str = sys.argv[3]
interaction_str = sys.argv[4]

rotation = (rotation_str == "on")
interaction = (interaction_str == "on")

if program == "ats":
    program = "ATS"
elif program == "dwc":
    program = "DWC"
elif program == "evt":
    program = "Evtloop"

if particle == "calib":
    runlist = [
        12171, 12172, 12183, 12182,  # -- M1, T1-4
        12174, 12175, 12181, 12180,  # -- M2, T1-4
        12176, 12177, 12179, 12178,  # -- M3, T1-4
        12184, 12185, 12195, 12194,  # -- M4, T1-4
        12186, 12187, 12193, 12192,  # -- M5, T1-4
        12188, 12189, 12191, 12190,  # -- M6, T1-4
        12196, 12197, 12207, 12206,  # -- M7, T1-4
        12198, 12199, 12205, 12204,  # -- M8, T1-4
        12200, 12201, 12203, 12202  # -- M9, T1-4
    ]
elif particle == "em":
    runlist = [
        12263, 12264, 12267, 12268, 12279, 12280, # 120 GeV
        12260, 12261, # 100 GeV
        12265, 12266, # 80 GeV
        12269, 12270, # 60 GeV
        12271, 12272, # 40 GeV
        12273, 12274, # 30 GeV
        12275, 12276, # 20 GeV
        12277, # 10 GeV
        12278 # 6 GeV
    ]
elif particle == "pi":
    if not rotation and not interaction:
        runlist = [
            12352, 12353, 12354, 12355, 12356, 12357, 12358, 12359, 12360, 12361, 12363, # 120GeV
            12364, 12365, 12366, 12367, 12368, 12369, 12370, 12371, 12372, 12373, # 100GeV
            12374, 12375, 12376, 12377, 12378, 12379, 12380, 12381, 12382, 12383, # 80GeV
            12384, 12385, 12386, 12387, 12388, # 60GeV
            12389, 12390, 12391, # 40GeV
            12392, 12393 # 20GeV
        ]
    elif rotation and not interaction:
        runlist = [
            12438, 12439, 12440, 12441, 12443, 12444, 12445, # 120GeV
            12432, 12433, 12434, 12435, 12436, 12437, # 100GeV
            12446, 12447, 12448, 12449, 12450, 12451, # 80GeV
            12452, 12453, 12454, 12455, 12456, # 60GeV
            12457, 12458, 12459, # 40GeV
            12460, 12461 # 20GeV
        ]
    elif not rotation and interaction:
        runlist = [
            12502, 12503, 12504, 12505, 12506, 12507, 12508, 12509, # 120GeV
            12510, 12511, 12512, 12513, 12514, 12515, # 100GeV
            12496, 12497, 12498, 12499, 12500, 12501, # 80GeV
            12516, 12517, 12518, 12519, 12520, # 60GeV
            12522, 12523, 12524, 12525 # 40GeV
        ]
    else:
        print("Invalid combination of rotation and interaction for pi particle.")
        sys.exit(1)
elif particle == "kaon" or particle == "proton":
    if not rotation and not interaction:
        runlist = [
            12401, 12402, 12403, 12404, 12405, 12406, 12407, 12408, # 120GeV
            12410, 12411, 12412, 12413, 12414, 12415, # 100GeV
            12416, 12417, 12418, 12419, 12420, 12421, # 80GeV
            12422, 12423, 12424, 12425, 12426, # 60GeV
            12427, 12428, 12429,# 40GeV
            12430, 12431 # 20GeV
        ]
    elif rotation and interaction:
        runlist = [
            12462, 12463, 12465, 12465, 12466, 12467, 12468, 12469, # 120GeV
            12470, 12471, 12472, 12473, 12474, 12475, # 100GeV
            12476, 12477, 12478, 12479, 12480, 12481, # 80GeV
            12482, 12483, 12484, 12485, 12486, # 60GeV
            12487, 12488, 12489, # 40GeV
            12490, 12491, 12492 # 20GeV
        ]
    elif not rotation and interaction:
        runlist = [
            12502, 12503, 12504, 12505, 12506, 12507, 12508, 12509, # 120GeV
            12510, 12511, 12512, 12513, 12514, 12515, # 100GeV
            12496, 12497, 12498, 12499, 12500, 12501, # 80GeV
            12516, 12517, 12518, 12519, 12520, # 60GeV
            12522, 12523, 12524, 12525 # 40GeV
        ]
    else:
        print("Invalid combination of rotation and interaction for kp particle.")
        sys.exit(1)
else:
    print(f"Invalid particle: {particle}")
    print("Usage: python Create_CondorJob.py particle:[calib|em|pi|kp] program:[ats|dwc|evt] rotation:[on|off] interaction target:[on|off]")
    sys.exit(1)
########################################
##           Configuration            ##
########################################
NumEvt = -1
centermodule = 5
version = "PA"
if rotation:
    version += "_" + "Rot"
if interaction:
    version += "_" + "IT"


for sample in range(len(runlist)):
    ### set up condordirectory ###
    save_dir = f"{particle}/{program}"
    sample_dir = f"{version}/{particle}/{program}/{runlist[sample]}"
    os.makedirs(sample_dir, exist_ok=True)
    os.makedirs(f"{sample_dir}/log_condor", exist_ok=True)
    ### create log_condor directory ###
    os.makedirs(f"{sample_dir}/log_condor/err", exist_ok=True)
    os.makedirs(f"{sample_dir}/log_condor/log", exist_ok=True)
    os.makedirs(f"{sample_dir}/log_condor/out", exist_ok=True)
    ### delete log_condor directory ###
    for f in os.listdir(f"{sample_dir}/log_condor/err"):
        os.remove(os.path.join(f"{sample_dir}/log_condor/err", f))
    for f in os.listdir(f"{sample_dir}/log_condor/log"):
        os.remove(os.path.join(f"{sample_dir}/log_condor/log", f))
    for f in os.listdir(f"{sample_dir}/log_condor/out"):
        os.remove(os.path.join(f"{sample_dir}/log_condor/out", f))
    ### created executive condor job: condor_run.sh ###
    with open(f"{sample_dir}/condor_run.sh", "w") as file:
        file.write("#!/bin/bash \n\n")
        file.write("source /u/user/gcho/DRC/TB2025/envset.sh\n\n")
        file.write("fileListNum=$((${1}+1)) \n")
        file.write("cd /u/user/gcho/DRC/TB2025/analysis \n")
        file.write("mkdir -p ./output/%s/\n" %(save_dir))
        if program == "ATS":
            file.write("./avgTimeStruc %s %s %s" % (runlist[sample], NumEvt, particle))
        elif program == "DWC":
            file.write("./draw_DWC %s %s %s" % (runlist[sample], NumEvt, particle))
        elif program == "Evtloop":
            file.write("./evtloop_DRC %s %s %s %s %s %s" % (runlist[sample], NumEvt, centermodule, particle, rotation_str, interaction_str))
        else:
            print(f"Invalid program: {program}")
    ### created submit condor job: condor_sub.sub ###
    with open(f"{sample_dir}/condor_sub.sub", "w") as file:
        file.write("universe = vanilla \n")
        file.write("executable = condor_run.sh \n\n")
        file.write("arguments = $(Process) \n")
        file.write("request_memory = 1024 MB \n\n")
        file.write("getenv = True \n")
        file.write("should_transfer_files = YES \n")
        file.write("when_to_transfer_output = ON_EXIT \n\n")
        file.write('Requirements = (machine =!= "cluster288.knu.ac.kr") && (machine =!= "cluster275.knu.ac.kr") && (machine =!= "cluster244.knu.ac.kr") && (machine =!= "cluster290.knu.ac.kr") && (machine =!= "dm01.knu.ac.kr") && (machine =!= "dm02.knu.ac.kr") && (TARGET.Arch == "X86_64") && (TARGET.OpSys == "LINUX") && (TARGET.HasFileTransfer)\n')
        file.write("output = /u/user/gcho/DRC/TB2025/analysis/Condor/%s/log_condor/out/out_$(Process).out \n" %(sample_dir) )
        file.write("error  = /u/user/gcho/DRC/TB2025/analysis/Condor/%s/log_condor/err/err_$(Process).err \n" %(sample_dir) )
        file.write("log    = /u/user/gcho/DRC/TB2025/analysis/Condor/%s/log_condor/log/log_$(Process).log \n\n" %(sample_dir) )
        file.write("JobBatchName = %s_%s \n" %(particle, version) )
        file.write("queue 1 \n")
    print("%s condor submit file created!!! " %(runlist[sample]))
print("----------------Parameters---------------\n")
print("  Particle: %s, Program: %s" %(particle, program))
print("  Rotation: %s, Interaction: %s" %(rotation_str, interaction_str))
print("-----------------------------------------\n")
